...
\begin{mybox}
    \lemma{Let $K$ be a number field of degree $n$ and let $\alpha, \beta \in K$ be algebraic numbers of degree $n$. Denote the conjugates of $\alpha$ as $\alpha_1, \ldots, \alpha_n$ and the conjugates of $\beta$ as $\beta_1, \ldots, \beta_n$. Then conjugates of $\alpha + \beta$ are $\alpha_{i} + \beta_{j}$ for $i, j = 1, \ldots n$. Similarly, the conjugates of $\alpha \beta$ are $\alpha_{i}\beta_{j}$.} 
\end{mybox}

\proof{Let $\sigma_1, \ldots, \sigma_n$ be the embeddings where $\sigma_i : K \hookrightarrow \C$ that fix $\Q$ and sends $\sigma_i(\alpha) = \alpha_i$. Then, $\sigma_i(\alpha + \beta) = \sigma_i(\alpha) + \sigma_i(\beta)$ and $\sigma_i(\alpha \beta) = \sigma_i(\alpha)\sigma_i(\beta)$ by homomorphism properties.\par}


\note{Let $K$ be a number field and let $\alpha \in K$. Then, let $\alpha_1, \ldots, \alpha_n$ be the conjugates of $\alpha$ in $K$. We write $\| \alpha \|$ to denote $\max \{\vert \alpha_1 \vert, \ldots, \vert \alpha_n \vert\}$.\par}

\begin{mybox}
    \lemma{$\|\alpha + \beta\| \leq \| \alpha \| + \| \beta \|$ and $\| \alpha\beta \| \leq \| \alpha \| \| \beta \|$.}
\end{mybox}
...
\begin{mybox}
    \lemma{Consider the following system of $m$ equations with $n$ unknowns with $0 < m < n$: 
    \[\begin{pmatrix}
        a_{1,1} & \cdots & a_{1,n} \\
        \vdots & \ddots & \vdots \\
        a_{m,1} & \cdots & a_{m,n} 
...
\proof{Let $h := [K : \Q]$ and let $\{\beta_1, \ldots, \beta_h\}$ be an integral basis for $K$. For $\alpha \in \mathcal{O}_K$ we can write $\alpha = g_1\beta_1 + \cdots + g_h\beta_h$.
\[ \greenlozenge \]}
