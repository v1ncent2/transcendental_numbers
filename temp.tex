...
\defn{Let $k, l$ be fields and $l/k$. Then, $a \in l$ is \vocab{algebraic} over $k$ if there is a nonzero polynomial $f \in k[x]$ such that $f(a) = 0$. Otherwise, $a$ is \vocab{transcendental} over $k$.}\par

\defn{Let $l/k$ be a field extension. We say that $X \subseteq L$ is \vocab{algebraically independent} over $k$ if for all $a_1, \ldots, a_t$ distinct and all $f \in k[x_1, \ldots, x_t]$, $f(a_1, \ldots, a_t) = 0$ implies $f = 0$.}

Now, we will consider the field extension $\C/\Q$.\par

\defn{An \vocab{algebraic number} is a complex number that is the root of a finite nonzero polynomial in one variable with rational coefficients. A \vocab{transcendental number} is a complex number that is not algebraic. In other words, an algebraic number is a complex number that is algebraic over $\Q$ and a transcendental number is a complex number that is not algebraic over $\Q$.}\par
...
Essentially, Liouville numbers are real numbers that can be approximately really, really closely by a rational $\frac{p}{q}$, where $q > 1$.\par

\begin{mybox}
    \theorem{(Liouville's Approximation Theorem) For any algebraic number $\alpha$ of degree $n > 2$, a rational approximation $\frac{p}{q}$ to $\alpha$, }
\end{mybox}
\proof{We will show that no algebraic number has this property. Namely, if $\alpha \in \A$, then for any approximation $\frac{p}{q}$ with $q > 1$, there will be some natural $n$ where $\vert \alpha - \frac{p}{q}\vert > \frac{1}{q^n}$.}
